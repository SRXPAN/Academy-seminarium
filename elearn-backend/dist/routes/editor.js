...
router.get('/topics', requireAuth, requireEditor, asyncHandler(async (_req, res) => {
...
router.get('/topics/:topicId/materials', requireAuth, requireEditor, validateResource(z.object({ topicId: z.string().cuid() }), 'params'), asyncHandler(async (req, res) => {
...
router.post('/topics/:topicId/materials', requireAuth, requireEditor, validateResource(z.object({ topicId: z.string().cuid() }), 'params'), validateResource(z.object({
...
router.put('/materials/:id', requireEditor, validateResource(materialSchemas.idParam, 'params'), validateResource(materialSchemas.updateTranslations, 'body'), asyncHandler(async (req, res) => {
...
router.delete('/topics/:topicId/materials/:id', requireAuth, requireEditor, validateResource(z.object({ topicId: z.string().cuid(), id: z.string().cuid() }), 'params'), asyncHandler(async (req, res) => {
    const topicId = getParam(req.params.topicId);
    const id = getParam(req.params.id);
...
// ==================== QUIZZES ====================
router.get('/topics/:topicId/quizzes', requireAuth, requireEditor, validateResource(z.object({ topicId: z.string().cuid() }), 'params'), asyncHandler(async (req, res) => {
    const topicId = getParam(req.params.topicId);
...
router.post('/topics/:topicId/quizzes', requireAuth, requireEditor, validateResource(z.object({ topicId: z.string().cuid() }), 'params'), validateResource(z.object({
    title: z.string().min(2),
...
router.put('/topics/:topicId/quizzes/:id', requireAuth, requireEditor, validateResource(z.object({ topicId: z.string().cuid(), id: z.string().cuid() }), 'params'), validateResource(z.object({
...
router.delete('/topics/:topicId/quizzes/:id', requireAuth, requireEditor, validateResource(z.object({ topicId: z.string().cuid(), id: z.string().cuid() }), 'params'), asyncHandler(async (req, res) => {
    const topicId = getParam(req.params.topicId);
    const id = getParam(req.params.id);
...
// ==================== QUESTIONS ====================
router.get('/quizzes/:quizId/questions', requireAuth, requireEditor, validateResource(z.object({ quizId: z.string().cuid() }), 'params'), asyncHandler(async (req, res) => {
...
router.post('/quizzes/:quizId/questions', requireAuth, requireEditor, validateResource(z.object({ quizId: z.string().cuid() }), 'params'), validateResource(z.object({
    text: z.string().min(5),
    ...
    const quizId = getParam(req.params.quizId);
    const hasCorrect = req.body.options.some((o) => o.correct);
    if (!hasCorrect)
        throw AppError.badRequest('At least one option must be correct');
...
router.put('/quizzes/:quizId/questions/:id', requireAuth, requireEditor, validateResource(z.object({ quizId: z.string().cuid(), id: z.string().cuid() }), 'params'), validateResource(z.object({
    ...
    if (req.body.textJson)
        questionData.textJson = req.body.textJson;
    if (req.body.explanation !== undefined)
        questionData.explanation = req.body.explanation;
    if (req.body.explanationJson)
        questionData.explanationJson = req.body.explanationJson;
    if (req.body.difficulty)
        questionData.difficulty = req.body.difficulty;
    if (req.body.tags)
        questionData.tags = req.body.tags;
...
router.delete('/quizzes/:quizId/questions/:id', requireAuth, requireEditor, validateResource(z.object({ quizId: z.string().cuid(), id: z.string().cuid() }), 'params'), asyncHandler(async (req, res) => {
    const quizId = getParam(req.params.quizId);
    const id = getParam(req.params.id);
 ...
 */
router.post('/ai/generate-quiz', requireAuth, requireEditor, validateResource(aiGenerateQuizSchema, 'body'), asyncHandler(async (req, res) => {
    ...
    const extractedText = await aiService.extractText(content, type);
    // Generate quiz using AI
    const generatedQuiz = await aiService.generateQuiz(extractedText, language);
 ...
 */
router.get('/ai/status', requireAuth, requireEditor, asyncHandler(async (_req, res) => {
    return ok(res, {
