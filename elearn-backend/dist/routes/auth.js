...
import { getAccessCookieOptions, getRefreshCookieOptions, getClearCookieOptions, getClearRefreshCookieOptions, } from '../utils/cookie.js';
import { registerUser, loginUser, refreshUserTokens, logoutUser, logoutAllDevices, changePassword, requestPasswordReset, resetPassword, verifyEmail, resendVerificationEmail, deleteUser, } from '../services/auth.service.js';
...
router.post('/login', authLimiter, validateResource(authSchemas.login, 'body'), asyncHandler(async (req, res) => {
    const { userAgent, ip } = getClientInfo(req);
...
router.put('/password', requireAuth, validateResource(authSchemas.changePassword, 'body'), asyncHandler(async (req, res) => {
    try {
...
router.post('/forgot-password', authLimiter, validateResource(authSchemas.forgotPassword, 'body'), asyncHandler(async (req, res) => {
...
router.post('/reset-password', authLimiter, validateResource(authSchemas.resetPassword, 'body'), asyncHandler(async (req, res) => {
...
router.post('/verify-email', validateResource(authSchemas.verifyEmail, 'body'), asyncHandler(async (req, res) => {
...
// PUT /api/auth/email — змінити email
router.put('/email', requireAuth, validateResource(authSchemas.changeEmail, 'body'), asyncHandler(async (req, res) => {
    const { newEmail, password } = req.body;
...
router.post('/avatar', requireAuth, asyncHandler(async (req, res) => {
    ...
    if (fileId && currentUser?.avatarId && fileId !== currentUser.avatarId) {
        if (currentUser.avatarFile?.key) {
            deleteFile(currentUser.avatarFile.key).catch((err) => logger.error('Failed to cleanup old avatar:', err));
        }
    }
    const updatedUser = await prisma.user.update({
        where: { id: req.user.id },
