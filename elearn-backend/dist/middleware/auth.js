...
function readToken(req) {
    ...
    if (typeof fromCookie === 'string' && fromCookie.length > 10)
        return fromCookie;
...
export async function requireAuth(req, res, next) {
    ...
    if (!token)
        return sendError(res, ErrorCodes.UNAUTHORIZED, 'No token', 401);
    try {
...
export function requireRole(roles) {
    return function (req, res, next) {
        ...
        if (!roles.includes(req.user.role))
            return sendError(res, ErrorCodes.FORBIDDEN, 'Forbidden', 403);
        next();
    };
}
/**
